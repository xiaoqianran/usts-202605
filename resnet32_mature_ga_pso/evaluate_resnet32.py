...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate standard ResNet32 checkpoint")
    ...
    loader = get_cifar10_test_loader(args.data_dir, args.batch_size, args.num_workers, args.max_test_samples, args.seed)
    loss, acc = evaluate(model, loader, nn.CrossEntropyLoss(), device)
    params = count_parameters(model)
    flops = measure_flops(model, input_size=(3, 32, 32), device=device)
    result = {"checkpoint": args.checkpoint, "model": "ResNet32", "test_loss": loss, "test_acc": acc, "params": params, "flops": flops}
