# MSTAR 深度半监督学习实践二实验代码

本项目针对上传的 `MSTAR.zip` 数据集设计三组对照实验：

1. **全标签监督学习**：使用 `mstar-train` 全部标签训练分类网络，在 `mstar-test` 测试。
2. **有限标签监督学习**：每类只保留训练样本的 10% 以下作为有标签数据，其余样本不参与监督训练，观察性能下降。
3. **半监督学习 FixMatch**：使用同样的 10% 以下有标签数据，同时把剩余训练样本作为无标签数据，通过一致性正则 + 伪标签训练，观察能否恢复精度。
