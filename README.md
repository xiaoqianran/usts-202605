...
<p align="center">
  <a href="#中文"><img src="https://img.shields.io/badge/中文-点击查看-red?style=for-the-badge" alt="中文"></a>
  <a href="#english"><img src="https://img.shields.io/badge/English-Click_to_view-blue?style=for-the-badge" alt="English"></a>
</p>
...
- 报告与答辩素材：7 张核心图表 + `practice2_section_2to3_pages.*`（docx/html/md，可直接插入报告第 2-3 节）
- 其他：中文字体支持、完整实验结果归档、`实验结果汇总.md`、`监控提示词.md`

**主要目录**：`mstar_ssl_practice2_clean/`（重写版，支撑完整实践二与答辩）

#### 实践三：深度学习应用实践（锂离子电池 SOH 预测）
...
<p align="center">
  <a href="#english"><b>→ 点击跳转到 English 版本</b></a>
</p>
...
#### Practice 1: PyTorch Deep Learning Basics
...
#### Practice 2: Deep Semi-Supervised Learning (MSTAR SAR Classification)
...
- Toolchain: one-click grid scripts (`run_supervised_grid.sh`, `run_fixmatch_grid.sh`), automatic result summarization tools, real-time monitoring scripts
- Report & defense assets: 7 core figures + `practice2_section_2to3_pages.*` (docx/html/md, ready to insert into reports)
- Additional: Chinese font support, full archived results, experiment summary and monitoring prompt documents

**Primary directory**: `mstar_ssl_practice2_clean/` (rewritten version supporting complete Practice 2 and defense)

#### Practice 3: Real-World DL Application (Li-ion Battery SOH Prediction)
...
#### Extended Experiments: Architecture Search & Compression
...
Each major experiment directory is largely self-contained and typically includes:
- `src/` — Core source code
- `scripts/` / `run_*.sh` — Experiment runners, monitoring, and one-click scripts
- `tools/` — Result summarization and asset generation utilities
- `requirements.txt` + `README.md`
- `runs/`, `outputs/`, `report_assets/` — Experimental results, metrics, figures, and defense materials

### Notes

- Large model weights (`.pt`), raw datasets (MSTAR, CIFAR), and some massive `runs/` directories are gitignored or managed separately as needed.
- See individual subdirectory READMEs for reproduction steps, design details, and key findings.
- Most experiments were run on NVIDIA H100 / L40S GPUs.
- This repository documents the complete course journey from basic implementations to large-scale hyperparameter studies, real-world application modeling, and neural architecture search.

---

<p align="center">
  <a href="#中文"><b>← 返回中文版本</b></a>
</p>

---

*本仓库 README 采用「中文优先 + 独立英文页」设计，顶部标签可快速跳转。*  
*This README uses a "Chinese-first + separate English page" design. Use the tabs at the top to switch between languages.*